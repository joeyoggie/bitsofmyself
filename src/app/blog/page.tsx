                ...
                {allPosts.map((post) => (
                    <article key={post.slug} className="group relative flex flex-col items-start">
                        <h2 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-gray-100">
                            <div className="absolute -inset-x-4 -inset-y-3 scale-95 opacity-0 transition group-hover:scale-100 group-hover:opacity-100 bg-gray-50 dark:bg-gray-800/50 sm:-inset-x-6 sm:rounded-2xl" />
                            <Link href={`/blog/${post.slug}`}>
                                <span className="absolute -inset-x-4 -inset-y-3 sm:-inset-x-6 sm:rounded-2xl" />
                                <span className="relative">{post.title}</span>
                            </Link>
                        </h2>
                        <div className="relative z-10 order-first mb-3 flex items-center text-sm text-gray-400 dark:text-gray-500 pl-3.5">
                        ...
                        <div aria-hidden="true" className="relative z-10 mt-4 flex items-center text-sm font-medium text-teal-600 dark:text-teal-400">
                            Read article
